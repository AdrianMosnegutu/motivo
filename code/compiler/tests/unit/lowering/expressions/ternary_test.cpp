...
TEST(TernaryEval, TrueConditionSelectsThenBranch) {
    const auto ir = lower_ok("track { int x = true ? 2 : 5; play A4 :x; play B4; }");
...
TEST(TernaryEval, FalseConditionSelectsElseBranch) {
    const auto ir = lower_ok("track { int x = false ? 2 : 5; play A4 :x; play B4; }");
    ASSERT_EQ(ir.tracks[0].events.size(), 2u);
...
TEST(TernaryEval, NestedTernaryInThenBranch) {
    // true ? (false ? 1 : 3) : 5  →  3
    const auto ir = lower_ok("track { int x = true ? (false ? 1 : 3) : 5; play A4 :x; play B4; }");
    ASSERT_EQ(ir.tracks[0].events.size(), 2u);
