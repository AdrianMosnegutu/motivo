...
This section documents how errors are collected across pipeline stages and the hard limits the implementation enforces
...
(\texttt{common/diagnostics/diagnostics\_engine.cpp}).
...
\topic{Stage gating in \texttt{compiler.cpp}.}
...
The Flex scanner (\texttt{scanner.l}) calls \texttt{report\_lexical}, which forwards to
...
error: semantic: <test>:1:11: time signature denominator must be a power of two between 1 and 32, got 3
error: semantic: <test>:2:14: undefined variable 'x'
error: semantic: <test>:3:9: left operand must be numeric
error: semantic: <test>:4:5: '&&' requires boolean operands
error: semantic: <test>:5:12: no matching overload of pattern 'motif'
error: lowering: division by zero
error: lowering: modulo by zero
error: lowering: program exceeds 20000 event limit (has 20002)
\end{lstlisting}
...
quantize poorly for extreme values) and matches the range covered by \texttt{metadata\_test.cpp}.
...
Integer and floating-point division in \texttt{evaluate\_binary\_expression.cpp} throw \texttt{LoweringFailure} when
...
\texttt{LowererContext::register\_events} increments a program-wide counter each time notes are emitted.
Exceeding \texttt{MAX\_EVENTS} aborts lowering with a diagnostic naming the limit and the actual count.
The cap serves two purposes: it bounds memory and MIDI file size for pathological loops (including \texttt{for (;;)}),
and it provides a practical termination guarantee where the language has no compile-time iteration bound.
