...
The public entry point \texttt{motivo::compile} orchestrates the full pipeline.
...
After each stage, the driver checks for stage-specific failures and returns early with the collected diagnostics rather
than proceeding on invalid input.
...
\topic{Text to tree (frontend).} The Flex scanner tokenizes the character stream; the Bison parser reduces tokens to an
\texttt{ast::Program}.
...
dispatch on \texttt{std::variant} node kinds.

\topic{Tree to timeline (lowering).} \texttt{detail::LowererContext} executes the annotated program like an
interpreter: statement lowerers emit \texttt{NoteEvent}s, expression evaluators resolve runtime \texttt{ir::Value}s, and
the value flattener expands chords and sequences into atomic events.

\topic{Timeline to binary (backend).} The MIDI backend converts absolute beat positions to ticks, splits each note
into Note-On and Note-Off messages, stable-sorts events, and writes delta-time encoded track chunks via
\texttt{midi::write\_midi}.
...
\begin{figure}[p]
  ...
  \motivoscalefig{%
    \begin{tikzpicture}[
        ...
        diag/.style={motivo/panel, fill=motivoRose!10, minimum width=2.2cm, minimum height=4cm, align=center}
      ]
      ...
      \node[diag, above=1cm of sem, yshift=0.15cm] (diagbox) {%
  ...
  \caption{Full compiler pipeline orchestrated by \texttt{motivo::compile}, including early-exit guards after each
    stage and the shared \texttt{DiagnosticsEngine} reference passed to every reporting
  site.}\label{fig:compiler-pipeline-full}
\end{figure}
