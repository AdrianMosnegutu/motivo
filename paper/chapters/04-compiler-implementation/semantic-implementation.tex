...
Type rules live in \texttt{motivo\_types} (\texttt{type\_rules.cpp}).
...
The diagnostics section below documents the full invariant rationale.

\subsection{Error Reporting Behaviour}\label{detail-subsec:semantic-error-behaviour}

\texttt{Traversal::diagnose} appends errors to the shared \texttt{DiagnosticsEngine} but does not abort the walk.
Independent errors in disjoint subtrees therefore appear in a single diagnostics vector, which is preferable for IDE
feedback.
When a subtree encounters an \texttt{Unknown} type (e.g., after an undefined identifier), downstream checks guard on
\texttt{is\_known} to avoid cascading false positives.
Stage gating in \texttt{compiler.cpp} ensures lowering never runs when any semantic error is present.
