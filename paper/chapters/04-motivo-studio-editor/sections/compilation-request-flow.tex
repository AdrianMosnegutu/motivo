...
When the user triggers compile (toolbar control or shortcut), \texttt{useIdeCompile} reads the current editor buffer
and calls \texttt{compileSource}.
...
On the server, Zod validates the body, then the compiler service writes \texttt{motivo-\{uuid\}.motivo} and
\texttt{motivo-\{uuid\}.mid} under the system temporary directory, runs \texttt{motivoc} with the source path
as its sole
argument, and deletes both files in a \texttt{finally} block.
Non-zero exit codes with stderr content are parsed line-by-line into diagnostics (lexical, syntax, semantic, lowering,
output stages) using a regex that strips ANSI color codes and optional \texttt{line:column} locations.
...
Compiler CTest and golden fixtures own language correctness end-to-end (Chapter~\ref{ch:deployment-ci},
Section~\ref{sec:testing-ci}).
