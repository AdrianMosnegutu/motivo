...
Pitch and instrument enums are defined in \texttt{motivo\_music}; binary serialization is implemented solely in
\texttt{midi/write\_midi.cpp} within the \texttt{motivo\_midi} CMake target, which depends on \texttt{motivo\_lowering}
for \texttt{ir::Program} and track data.
...
\noindent \textbf{\texttt{build\_tempo\_track}.} Emits tempo (\texttt{0xFF 0x51 0x03}) and time-signature
(\texttt{0xFF 0x58 0x04}) meta-events at tick~0, then an end-of-track marker.

\noindent \textbf{\texttt{build\_motivo\_track}.} Selects MIDI channel~9 (zero-based index~9, conventionally displayed
...
\noindent \textbf{\texttt{write\_track}.} Stable-sorts \texttt{MidiEvent} records by absolute tick, converts to
