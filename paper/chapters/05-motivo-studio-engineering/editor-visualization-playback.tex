...
\topic{Example library.} Bundled \texttt{.motivo} files under \texttt{features/examples/sources} (including
\texttt{example.motivo}, \texttt{fur\_elise.motivo}, \texttt{pirates.motivo}, \texttt{come\_as\_you\_are.motivo}) open
as read-only tabs for demonstration; guests can compile and play them but cannot persist edits to the database.
...
\texttt{useFileWorkspace} tracks dirty state per user-owned document; \texttt{saveActiveDocument} persists
the active tab
via \texttt{PATCH /api/files/:id} on explicit save (toolbar button or Ctrl/Cmd+S).
