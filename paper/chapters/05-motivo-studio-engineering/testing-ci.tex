...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}M{2.1cm}M{3.0cm}rM{3.6cm}@{}}
    ...
    \midrule
    Compiler & CTest, GoogleTest & 652 & Unit, integration, golden \\
    Server & Vitest, Supertest & 31 & Unit, integration \\
    Client & Vitest, Testing Library & 88 & Unit, integration \\
    \bottomrule
  \end{tabularx}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}M{2.6cm}>{\raggedright\arraybackslash\hspace{0pt}}X@{}}
    ...
    Paper & format, lint, and spell checks $\rightarrow$ PDF build. \\
    \bottomrule
  \end{tabularx}
...
\begin{figure}[htbp]
  ...
  paper workflow gates the PDF build on format, lint, and spell checks.}\label{fig:ci-workflows}
\end{figure}
...
The compiler and the studio that surrounds it are now both built, tested, and deployed.
What remains is to ask whether the central claim behind the design actually holds: that compact symbolic source expands
deterministically into much larger MIDI timelines, fast enough and cheaply enough for interactive use.
The next chapter answers that empirically, with benchmarks on the compiler corpus.
