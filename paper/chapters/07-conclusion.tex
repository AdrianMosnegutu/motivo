...
This thesis presented the design, implementation, and evaluation of a novel Domain-Specific Language (DSL) for musical
composition.
...
maps high-level musical intent into a static, perfectly quantized Intermediate Representation (IR).

Key achievements include:
\begin{itemize}
  \item \textbf{Structural Abstraction:} The language provides robust constructs (\texttt{pattern}, \texttt{track},
  ...
  \item \textbf{Deterministic Lowering:} The implementation of a stateful cursor mutation algorithm successfully
    ``unrolls'' C-style imperative control flow (loops, conditionals) and recursive pattern calls into a linear sequence
    of MIDI events, while perfectly preserving temporal gaps and rests.
  \item \textbf{Type-Safe Semantics:} The custom semantic analyzer rigorously enforces musical logic, ensuring that
  ...
  \item \textbf{Robust Tooling:} The development of a full-stack, web-based playground with real-time diagnostics and
...
\begin{itemize}
  \item \textbf{Lack of Real-Time Interaction:} Because the output is fully resolved at compile-time into a static file,
    the language cannot respond to external input (such as live MIDI controllers or network signals) during playback.
  \item \textbf{MIDI Expressiveness Constraints:} The current implementation is bound by the constraints of the MIDI 1.0
    standard.
  ...
  \item \textbf{Static Pattern Signatures:} Pattern parameters currently only accept concrete evaluated types.
...
Constructs}\label{subsec:standard-library-and-advanced-musical-constructs}
...
resolving Roman numeral chord progressions) would elevate the DSL to a higher level of musical abstraction.

\subsection{Continuous Parameter Automation}\label{subsec:continuous-parameter-automation}
...
\subsection{Alternative Backends}\label{subsec:alternative-backends}
...
DSL empowers composers to explore complex structural arrangements with precision, confidence, and speed.
