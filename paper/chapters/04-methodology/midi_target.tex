...
While some musical DSLs target raw audio synthesis or real-time event streaming via Open Sound Control (OSC), this
research utilizes the \textit{MIDI Format 1} protocol as its primary build target.

\subsection{Interoperability and the Producer Workflow}\label{subsec:interoperability-and-the-producer-workflow}

A key goal of this DSL is to serve as a tool for \textit{producers} and \textit{composers}, not just a self-contained
instrument.
By targeting MIDI, the compiler produces an artifact that is natively understood by almost every Digital Audio
Workstation (DAW) and notation software in existence.
...
\subsection{Preserving Symbolic Identity}\label{subsec:preserving-symbolic-identity}
...
This makes MIDI an ideal target for a structured DSL because the high-level constructs of the language (e.g., a ``C4''
note at beat 2) map directly to low-level MIDI messages (\texttt{Note-On, key 72, velocity 100}).
This one-to-one mapping ensures that the semantic intent of the composer is perfectly preserved in the output file,
allowing for further offline analysis or modification in a DAW\@.

\subsection{Offline Analysis and Reproducibility}\label{subsec:offline-analysis-and-reproducibility}
...
This file can be archived, version-controlled, and shared, guaranteeing that the composition remains ``frozen'' exactly
