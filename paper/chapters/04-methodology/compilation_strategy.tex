...
\subsection{The Zero-Runtime Mandate}\label{subsec:the-zero-runtime-mandate}
...
\begin{itemize}
  \item \textbf{High Performance:} The output artifact is extremely lightweight, as all logical overhead has been
    removed.
  \item \textbf{Offline Validation:} Semantic errors—such as variable shadowing, type mismatches, or invalid rhythmic
    math—are caught during the compilation phase, providing immediate feedback to the composer.
  \item \textbf{Portability:} The resulting IR can be easily serialized into multiple formats (such as MIDI or JSON)
    without requiring a specialized interpreter to read it.
\end{itemize}

\subsection{Temporal Reconstruction of Patterns}\label{subsec:temporal-reconstruction-of-patterns}
...
\subsection{Static Verification of Compositional Logic}\label{subsec:static-verification-of-compositional-logic}
