...
subdivisions).
The proposed DSL utilizes a deterministic \textit{cursor model} to manage temporal placement, providing a balance
between sequential flow and absolute positioning.

\subsection{The Virtual Playhead}\label{subsec:the-virtual-playhead}
...
\begin{enumerate}
  \item \textbf{Relative Advancement:} Standard \texttt{play} and \texttt{rest} statements are relative to the current
    cursor position.
  ...
  \item \textbf{Absolute Offsets:} The \texttt{from} keyword allows for non-destructive positioning.
...
\subsection{Parallelism and Temporal Isolation}\label{subsec:parallelism-and-temporal-isolation}
...
\subsection{Deterministic Time Resolution}\label{subsec:deterministic-time-resolution}
...
this DSL ensures that every event is calculated to an exact byte-offset in the output file.
