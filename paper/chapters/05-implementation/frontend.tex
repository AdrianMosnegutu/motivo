...
\subsection{Lexical Analysis: Identifying Musical Literals}\label{subsec:lexical-analysis:-identifying-musical-literals}
...
\begin{lstlisting}[caption={Flex regular expression for note literal recognition.},label={lst:lstlisting}]
pitch        [A-G]
accidental   [#b]
octave       [0-8]
...
\subsection{Syntactic Analysis: Building the Hierarchy}\label{subsec:syntactic-analysis:-building-the-hierarchy}
...
\begin{itemize}
  ...
  \item \textbf{\texttt{PatternDefinition}:} A node that captures the signature of a musical function, including its
    name, parameter list, and a block of statements.
  \item \textbf{\texttt{VoiceDefinition}:} Represents horizontal parallelism.
...
\begin{itemize}
  ...
  \item \textbf{\texttt{ExpressionKind}:} A variant containing literals (\texttt{Int}, \texttt{Float}, \texttt{Bool},
