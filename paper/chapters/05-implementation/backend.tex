...
\subsection{SMF Format 1 Structure}\label{subsec:smf-format-1-structure}
...
\begin{itemize}
  \item \textbf{\texttt{MThd} (Header Chunk):} The file begins with a header specifying \texttt{Format=1}
    ...
    DSL source) to account for the mandatory tempo track.
  ...
  \item \textbf{\texttt{MTrk} (Track Chunks):} The first track (Track 0) is initialized as the ``Tempo Track''.
    ...
    Following the tempo track, each DSL track is serialized into its own \texttt{MTrk} chunk.
\end{itemize}

\subsection{The Delta-Time Sorting Algorithm}\label{subsec:the-delta-time-sorting-algorithm}
...
\begin{enumerate}
  \item \textbf{Tick Conversion:} Every absolute beat position and duration in the IR is multiplied by the PPQ constant
  ...
  \item \textbf{Event Separation:} The backend transforms each \texttt{NoteEvent} into two separate low-level events: a
    \texttt{Note-On} event at the start tick, and a \texttt{Note-Off} event at the end tick.
  \item \textbf{Stable Sorting:} All separated events within a track are collected into a vector and sorted using
    \texttt{std::ranges::stable\_sort} by their absolute tick value.
    The stable sort guarantees that if a \texttt{Note-Off} and a \texttt{Note-On} for the same pitch occur at the exact
    same tick, their relative ordering is preserved, preventing overlapping silence issues.
  \item \textbf{Delta Computation:} The backend iterates sequentially through the sorted list.
    For each event, the delta is computed as the difference between \texttt{current\_tick} and
    \texttt{previous\_tick}.
\end{enumerate}

\subsection{Variable-Length Quantities (VLQ)}\label{subsec:variable-length-quantities-(vlq)}
...
\subsection{Event Emission}\label{subsec:event-emission}

Finally, the backend emits the actual status and data bytes for every event:
\begin{itemize}
  \item \textbf{Program Change (\texttt{0xC0 | channel}):} Emitted at tick 0 for every melodic track to assign the
    chosen instrument.
  \item \textbf{Note-On (hexadecimal \texttt{90} with the channel encoded in the low nibble):} Contains the pitch and
    the defined velocity.
  \item \textbf{Note-Off (hexadecimal \texttt{80} with the channel encoded in the low nibble):} Explicitly emitted with
    a velocity of 0 to terminate the sound.
  \item \textbf{End of Track (\texttt{0xFF 2F 00}):} A mandatory meta-event appended to the end of every \texttt{MTrk}
