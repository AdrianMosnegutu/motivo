...
parameterized algorithms while guaranteeing that the resulting temporal structure is mathematically precise, free of
real-time execution jitter, and universally readable by standard production software.

\section{Proposed Solution}\label{sec:proposed-solution}

This thesis proposes the design and implementation of a novel, compiled Domain-Specific Language (DSL) for musical
composition.
...
\begin{enumerate}
  \item \textbf{Music as a Recursive Hierarchy:} The language abstracts musical motifs into functions (patterns) that
    can be parameterized, reused, and nested, reflecting the true hierarchical nature of musical form.
  \item \textbf{C-Style Imperative Flow:} By utilizing familiar control flow structures (\texttt{for}, \texttt{loop},
    \texttt{if}), the DSL allows composers to procedurally generate variations and repetitions without learning esoteric
    functional paradigms.
  \item \textbf{Zero-Runtime Compilation:} The compiler operates strictly offline.
    ...
    This guarantees deterministic temporal resolution without the overhead of an interpreter.
  \item \textbf{MIDI as a Semantic Target:} The compiler serializes the final timeline into a Standard MIDI File
    (Format 1).
    This ensures absolute interoperability, allowing the generated compositions to be imported into any DAW for
...
\begin{itemize}
  \item \textbf{Chapter 2 (Background):} Provides the foundational knowledge required to understand the system,
    including a primer on music theory, the technical specifications of the MIDI protocol, and the basic stages of
    compiler construction.
  \item \textbf{Chapter 3 (Related Work):} Analyzes existing music programming environments and low-level synthesis
    languages, highlighting the design gaps that motivate this research.
  \item \textbf{Chapter 4 (Methodology):} Details the conceptual framework of the DSL, explaining the mapping of musical
    primitives to programming constructs and the rationale behind the compiler's temporal logic.
  \item \textbf{Chapter 5 (Implementation):} Offers a deep technical dive into the C++23 codebase.
    It covers the Flex/Bison frontend, the recursive semantic analyzer, the stateful lowering engine, and the binary
    MIDI serializer.
    It also discusses the architecture of the web-based demonstration playground and the verification suite.
  \item \textbf{Chapter 6 (Evaluation):} Assesses the performance and utility of the compiler, focusing on execution
    speed, memory footprint, and the expressiveness (compression ratio) of the language compared to manual event
    placement.
  \item \textbf{Chapter 7 (Conclusion):} Summarizes the contributions of the thesis, discusses its current limitations,
    and outlines potential avenues for future research and expansion.
\end{itemize}
