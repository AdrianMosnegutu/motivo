...
By combining control flow with explicitly typed variables (e.g.\ \texttt{int i = 0}), a composer can write code that
generates variations of a theme without manually copying and pasting events.
...
\subsection{Explicit Types in Imperative Programs}\label{detail-subsec:explicit-types-syntax}

Version~2.0 requires a type keyword on every new binding (\texttt{int}, \texttt{double}, \texttt{bool},
\texttt{note}, \texttt{seq}, or \texttt{chord}) and on each pattern parameter.
This mirrors mainstream statically typed languages and aligns the surface syntax with the compiler's
\texttt{types::Type} enum, so IDE completion and semantic diagnostics can reference the same type names the author
writes.
The small syntactic cost buys unambiguous pattern overloading and eliminates the deferred errors that version~1.0's
second semantic pass was introduced to catch.
